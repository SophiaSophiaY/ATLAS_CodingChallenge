...
class AtlasAnnotationTool(QWidget):
    def __init__(self, app):
        ...
        # scene variable -- crop specific buttons
        self.btn_crop_selected = None
        self.btn_crop_floodfill = None
        ...
        self.input_p2y = None
        self.input_p2z = None
        self.btn_crop_cuboid = None
        self.btn_crop_next = None
        self.btn_crop_restore = None
    ...
    def setOnClickListener(self):
        ...
        self.btn_crop_selected.clicked.connect(self.btn_crop_selected_clicked)
        self.btn_crop_floodfill.clicked.connect(self.btn_crop_floodfill_clicked)
        self.btn_clear_selection.clicked.connect(self.btn_clear_selection_clicked)
        self.btn_crop_cuboid.clicked.connect(self.btn_crop_cuboid_clicked)
        self.btn_crop_next.clicked.connect(self.btn_crop_next_clicked)
        self.btn_crop_restore.clicked.connect(self.btn_crop_restore_clicked)
    ...
    def wireWidgets(self):
        ...
        #Crop wiring
        tab_crop = self.base_form.system_mode_layout.itemAt(0).widget().widget(1)
        self.btn_crop_selected = tab_crop.children()[1]
        self.btn_crop_floodfill = tab_crop.children()[2]
        self.btn_clear_selection = tab_crop.children()[3]
        self.input_p1x = tab_crop.children()[5]
        self.input_p1y = tab_crop.children()[7]
        self.input_p1z = tab_crop.children()[9]
        self.input_p2x = tab_crop.children()[11]
        self.input_p2y = tab_crop.children()[13]
        self.input_p2z = tab_crop.children()[15]
        self.btn_crop_cuboid = tab_crop.children()[16]
        self.btn_crop_next = tab_crop.children()[17]
        self.btn_crop_restore = tab_crop.children()[18]
    ...
    def btn_floodfill_done_clicked(self):
        ...
        self.selected_points_id = []
        self._refreshTopCanvasWithSelection()
    ...
    def btn_crop_selected_clicked(self):
        try:
            surface_to_crop = floodfill(self.selected_points_id, self.upperScene.pcd)
            self.current_result_point_indices = surface_to_crop #FIXME
            new_pcd = crop_remove(self.upperScene.pcd, self.selected_points_id)
            self.lowerScene.render(new_pcd)
    ...
    def btn_crop_floodfill_clicked(self):
        try:
            surface_to_crop = floodfill(self.selected_points_id, self.upperScene.pcd)
            self.current_result_point_indices = surface_to_crop #FIXME
            new_pcd = crop_remove(self.upperScene.pcd, surface_to_crop)
            self.lowerScene.render(new_pcd)
    ...
    def btn_clear_selection_clicked(self):
        self.writeMessage("Selected Points is cleared".format(len(self.selected_points_id)))
        self.selected_points_id = []
        self._refreshTopCanvasWithSelection()

    def btn_crop_cuboid_clicked(self):
        try:
            p1x = float(self.input_p1x.text())
            p1y = float(self.input_p1y.text())
            p1z = float(self.input_p1z.text())
            p2x = float(self.input_p2x.text())
            p2y = float(self.input_p2y.text())
            p2z = float(self.input_p2z.text())
        except Exception as e:
            self.writeMessage(str(e))
        self.writeMessage("Cropping points in cubiod:({},{},{},{},{},{})".format(p1x, p1y, p1z, p2x, p2y, p2z))

        px, py, pz = (p1x, p2x), (p1y, p2y), (p1z, p2z)
        minx, maxx = min(px), max(px)
        miny, maxy = min(py), max(py)
        minz, maxz = min(pz), max(pz)
        points = np.asarray(self.upperScene.pcd.points)
        surface_to_crop = [p[0] for p in enumerate(points) if minx < p[1][0] < maxx and miny < p[1][1] < maxy and  minz < p[1][2] < maxz]
        
        self.writeMessage("{} points cropped in cubiod".format(len(surface_to_crop)))
        new_pcd = crop_remove(self.upperScene.pcd, surface_to_crop)
        self.lowerScene.render(new_pcd)

    def btn_crop_next_clicked(self):
        self.upperScene.render(self.lowerScene.pcd)

    def btn_crop_restore_clicked(self):
        self.writeMessage("Opening file <{}>".format(self.current_data_file_name))
        self.upperScene.render(o3d.io.read_point_cloud(self.current_data_file_name))
        self.lowerScene.clear()
    ...
    def _refreshTopCanvasWithSelection(self):
        pcd = self.upperScene.pcd
        points = np.asarray(pcd.points)
        colors = np.asarray(pcd.colors)
        pid = np.asarray(self.selected_points_id, dtype=int)
        sizes = np.full(len(points), self.point_size)
        np.put(sizes, pid, 15)
        colors_with_selection = np.copy(colors)
        np.put(colors_with_selection, pid * 3 + 0, 1.0)
        np.put(colors_with_selection, pid * 3 + 1, 0.0)
        np.put(colors_with_selection, pid * 3 + 2, 0.0)
        self.upperScene.marker.set_data(points, edge_color=colors_with_selection, face_color=colors_with_selection, size=sizes)
        ...
        if event.button == 1 and self.distance_traveled(event.trail()) <= 2:
            ...
            if idx > 0:
                try:
                    ...
                    self.writeMessage("Selected Points Coords: {}".format(points[self.selected_points_id]))
                except IndexError:
